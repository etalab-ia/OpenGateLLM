...
def provider_settings_form_fields() -> rx.Component:
    return rx.grid(
        ...
        entity_form_input_field(
            ...
            value=ProvidersState.entity.max_context_length,
            tooltip="Max context length of the model, probed on the provider API",
            placeholder="No max context length",
            read_only=True,
        ),
        entity_form_input_field(
            label="Vector size",
            value=ProvidersState.entity.vector_size,
            tooltip="Vector size of the model, probed on the provider API for text embeddings inference models only",
