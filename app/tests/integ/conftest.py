...
def roles(test_client: TestClient) -> tuple[dict, dict]:
    ...
    # create role user
    response = test_client.post(url=ENDPOINT__ADMIN_ROLES, json={"name": "test-role-user", "default": False, "permissions": [], "limits": limits})
    logging.debug(msg=f"create role test-role-user: {response.text}")
    response.raise_for_status()
    role_id_without_permissions = response.json()["id"]

    response = test_client.get(url=f"{ENDPOINT__ADMIN_ROLES}/{role_id_with_permissions}")
    logging.debug(msg=f"get role test-role-with-permissions: {response.text}")
    response.raise_for_status()
    role_with_permissions = response.json()

    response = test_client.get(url=f"{ENDPOINT__ADMIN_ROLES}/{role_id_without_permissions}")
...
def users(test_client: TestClient, roles: tuple[dict, dict]) -> tuple[dict, dict]:
    ...
    # create user admin
    response = test_client.post(
        url=ENDPOINT__ADMIN_USERS, json={"name": "test-user-admin", "password": "test-password", "role": role_with_permissions["id"]}
    )
    response.raise_for_status()
    user_id_with_permissions = response.json()["id"]

    response = test_client.get(url=f"{ENDPOINT__ADMIN_USERS}/{user_id_with_permissions}")
    ...
    # create user user
    response = test_client.post(
        url=ENDPOINT__ADMIN_USERS, json={"name": "test-user-user", "password": "test-password", "role": role_without_permissions["id"]}
    )
    response.raise_for_status()
    user_id_user = response.json()["id"]

    response = test_client.get(url=f"{ENDPOINT__ADMIN_USERS}/{user_id_user}")
...
def tokens(test_client: TestClient, users: tuple[dict, dict]) -> tuple[dict, dict]:
    ...
    # create token admin
    response = test_client.post(
        url=ENDPOINT__ADMIN_TOKENS, json={"user": user_with_permissions["id"], "name": "test-token-admin", "expires_at": int(time.time()) + 300}
    )
    response.raise_for_status()
    ...
    # create token user
    response = test_client.post(
        url=ENDPOINT__ADMIN_TOKENS, json={"user": user_without_permissions["id"], "name": "test-token-user", "expires_at": int(time.time()) + 300}
    )
    response.raise_for_status()
