...
The REST API documentation can be found on [etalab.gouv.fr](https://etalab.gouv.fr/). The full API of this library can be found in [api.md](api.md).
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `albert_api.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `albert_api.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `albert_api.APIError`.
...
except albert_api.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except albert_api.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except albert_api.APIStatusError as e:
...
You can enable logging by setting the environment variable `ALBERT_API_LOG` to `info`.

```shell
$ export ALBERT_API_LOG=info
```
...
```python
import httpx
from albert_api import AlbertAPI, DefaultHttpxClient

client = AlbertAPI(
    # Or use the `ALBERT_API_BASE_URL` env var
