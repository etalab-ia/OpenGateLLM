        ...
        *,
        ...
        Args:
          input: Input text to embed, encoded as a string or array of tokens. To embed multiple
              ...
              The input must not exceed the max input tokens for the model (call `/v1/models`
              endpoint to get the `max_context_length` by model) and cannot be an empty
              string.

          model: ID of the model to use. Call `/v1/models` endpoint to get the list of available
              models, only `text-embeddings-inference` model type is supported.

          dimensions: The number of dimensions the resulting output embeddings should have.

          encoding_format: The format of the output embeddings. Only `float` is supported.
