...
from ui.backend.document_parsing import parse_document, count_document_characters, extract_text_from_parsed_document, process_large_document
...
with st.sidebar:
        ...
        if st.button("🔄 Traiter le document", use_container_width=True):
            try:
                ...
                if char_count < 10000:
                    ...
                    st.session_state["document_context"] = {"filename": uploaded_file.name, "content": document_text, "char_count": char_count}
    ...
    params["sampling_params"]["model"] = st.selectbox(
        ...
        options=models,
        index=models.index(f"{configuration.playground.default_model}") if f"{configuration.playground.default_model}" in models else 0,
    )
