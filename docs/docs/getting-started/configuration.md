...
<<<<<<< HEAD
=======
| swagger_title | string | Display title of your API in swagger UI, see https://fastapi.tiangolo.com/tutorial/metadata for more information. |  | OpenGateLLM |  | My API |
>>>>>>> f2cf455 (feat: model provision by SQL)
...
| load_balancing_strategy | string | Routing strategy for load balancing between providers of the model. It will be used to identify the model type. |  | shuffle | • shuffle<br></br>• least_busy | round_robin |
