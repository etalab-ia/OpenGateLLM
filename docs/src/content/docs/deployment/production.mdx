...
- Restrict network access to `/v1/auth/sso/login` so only the Playground can reach it.
  The API validates the oauth2-proxy session by calling `{auth_playground_url}/oauth2/auth` instead of re-validating OIDC tokens with the identity provider.
  So, configure `auth_playground_url` in the API configuration file with internet accessible Playground URL (eg. `http://playground:8501` in Docker network).

  For example with Nginx configuration:
  ```nginx
  location ~ ^/v1/auth/sso/login(/|$) {
    return 404;
  }
  ```
