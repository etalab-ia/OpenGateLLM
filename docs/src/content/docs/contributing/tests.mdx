...
The [Testing](https://github.com/etalab-ia/OpenGateLLM/blob/main/.github/workflows/run_tests.yml) workflow selects jobs from the pull request state and the trigger type.
...
<Aside type="note">
Jobs skip when the actor is `github-actions[bot]`, so the coverage badge commit does not retrigger the workflow.
</Aside>