...
When `usage_source` is `langfuse`, environmental impacts (`kWh`, `kgCO2eq`) are stored as Langfuse numeric scores and aggregated back into the daily usage buckets with a dedicated query, so `GET /v1/usage` reports them the same way as with PostgreSQL.
