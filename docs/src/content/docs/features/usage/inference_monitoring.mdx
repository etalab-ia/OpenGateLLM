...
By default, usage is stored in **PostgreSQL**. You can replace that store with **Langfuse** by configuring the optional Langfuse dependency. Only one backend is used at a time: when Langfuse is connected, new requests are not written to the `usage` table.
