...
`auth_sso_default_organization_id` must point at an existing organization — every user belongs to one, see the [organizations documentation](/features/users_management/organizations/).
...
8. If the user passes the access filters, the API endpoint `/v1/auth/sso/login` creates the user when needed, then returns a fresh API key for playground access (hidden in the playground).
