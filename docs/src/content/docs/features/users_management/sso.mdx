...
To customize this behavior, override the use case file in the API container. The class exposes hook methods you can replace:

- `async has_access(claims) -> bool` — Whether the user may access the Playground after OIDC authentication. Returning `False` yields `403`.
- `async get_user_name(claims) -> str \| None` — Display name stored on the user.
- `async get_organization_id(claims) -> int` — OpenGateLLM organization assigned on create/update.
- `async get_role_id(claims) -> int` — OpenGateLLM role assigned on create/update.

Mount your customized file over the built-in use case path. Example with Docker Compose:
