...
def use_case(provider_repository, user_with_role_query):
    return DeleteProviderUseCase(provider_repository=provider_repository, user_with_role_query=user_with_role_query)
...
class TestDeleteProviderUseCase:
    @pytest.mark.asyncio
    async def test_should_return_deleted_provider_when_user_is_admin_and_provider_exists(
        ...
        # Act
        result = await use_case.execute(command=DeleteProviderCommand(user_id=admin_user.id, provider_id=42))
    ...
    async def test_should_return_provider_not_found_error_when_provider_does_not_exist(self, use_case, provider_repository, admin_user):
        ...
        # Act
        result = await use_case.execute(command=DeleteProviderCommand(user_id=admin_user.id, provider_id=99))
    ...
    async def test_should_return_user_is_not_admin_error_when_user_is_not_admin(self, use_case, provider_repository, non_admin_user):
        ...
        # Act
        result = await use_case.execute(command=DeleteProviderCommand(user_id=non_admin_user.id, provider_id=42))

        # Assert
        assert isinstance(result, UserIsNotAdminError)
        provider_repository.delete_provider.assert_not_called()
    ...
    async def test_should_return_user_expired_error_when_user_expired(self, use_case, expired_user):
        ...
        # Act
        result = await use_case.execute(command=DeleteProviderCommand(user_id=expired_user.id, provider_id=42))
