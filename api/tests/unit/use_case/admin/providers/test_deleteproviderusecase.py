...
def use_case(provider_repository, user_info_repository):
    return DeleteProviderUseCase(provider_repository=provider_repository, user_info_repository=user_info_repository)
...
class TestDeleteProviderUseCase:
    ...
    async def test_should_return_deleted_provider_when_user_is_admin_and_provider_exists(
        ...
        # Act
        result = await use_case.execute(command=DeleteProviderCommand(user_id=admin_user_info.id, provider_id=42))
    ...
    async def test_should_return_user_is_not_admin_error_when_user_is_not_admin(self, use_case, provider_repository, unauthorized_user_info):
        ...
        # Act
        result = await use_case.execute(command=DeleteProviderCommand(user_id=unauthorized_user_info.id, provider_id=42))
