...
def provider_user_info():
    return UserWithRoleFactory(id=2, permissions=[PermissionType.PROVIDE_MODELS], limits=[])
...
class TestGetOneRouterUseCase:
    @pytest.mark.asyncio
    async def test_should_return_router_when_user_is_admin(self, use_case, router_repository, user_with_role_query, admin_user_info, sample_router):
        # Arrange
