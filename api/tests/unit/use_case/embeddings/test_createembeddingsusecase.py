...
def use_case(
    ...
    provider_client.forward_request.return_value = ProviderOriginalResponse(data=VllmEmbeddingsResponseFactory())
...
def mock_embeddings_latency_120ms():
    with patch(
        "api.use_cases.embeddings._createembeddingsusecase.time.perf_counter",
        side_effect=[0, 0.12],
    ):
        yield
