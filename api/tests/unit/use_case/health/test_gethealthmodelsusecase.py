...
class TestGetHealthModelsUseCase:
    async def test_should_return_green_for_live_uncapped_provider(
        ...
        assert mock_provider_client.forward.await_args.kwargs["request"].endpoint == ProviderEndpoint.MODELS
    ...
    async def test_should_prefer_yellow_over_red(
        self,
        use_case,
        command,
        mock_router_repository,
        ...
        router = RouterFactory(id=1, name="model", providers=2)
        red_provider = ProviderFactory(id=1, router_id=router.id, qos_limit=1)
        yellow_provider = ProviderFactory(id=2, router_id=router.id, qos_limit=20)
        mock_router_repository.get_all_routers.return_value = [router]
        mock_provider_repository.get_all_providers.return_value = [red_provider, yellow_provider]
        mock_provider_qos.get_loads.return_value = {red_provider.id: 1, yellow_provider.id: 19}
        mock_provider_client.forward.return_value = ProviderResponse(id="request-2", data=Models(data=[]))
    ...
    async def test_should_skip_routers_with_no_providers(
        self,
        use_case,
        mock_router_repository,
        mock_provider_repository,
        ...
        user = AuthenticatedUserFactory(
            ...
            limits=[
                ...
                Limit(router_id=2, value=100, type=LimitType.RPM),
            ],
            permissions=[],
        )
        command = GetHealthModelsCommand(authenticated_user=user)
        empty = RouterFactory(id=1, name="empty", providers=0)
        live = RouterFactory(id=2, name="model", providers=1)
        provider = ProviderFactory(id=2, router_id=live.id, qos_limit=None)
        mock_router_repository.get_all_routers.return_value = [empty, live]
        mock_provider_repository.get_all_providers.return_value = [provider]
        mock_provider_qos.get_loads.return_value = {provider.id: 0}
        ...
        # Assert
        assert result.models == [ModelHealthStatus(id="model", status=HealthStatus.GREEN)]
