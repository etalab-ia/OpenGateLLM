...
@pytest.mark.asyncio
async def test_login_success_master(session: AsyncSession, iam: IdentityAccessManager):
    token_id, token = await iam.login(session=session, email="master", password="secret")
    assert token_id == 0
    assert token == "secret"


@pytest.mark.asyncio
async def test_login_success_user(session: AsyncSession, iam: IdentityAccessManager):
    ...
    token_id, token = await iam.login(session=session, email="alice@example.com", password=password_plain)
    assert token_id == 123
    assert token == "tok-xyz"


@pytest.mark.asyncio
async def test_login_user_not_found(session: AsyncSession, iam: IdentityAccessManager):
    # Make get_user_info raise the expected exception
    iam.get_user_info = AsyncMock(side_effect=UserNotFoundException())

    with pytest.raises(UserNotFoundException):
        await iam.login(session=session, email="nonexistent@example.com", password="whatever")


@pytest.mark.asyncio
async def test_login_wrong_password(session: AsyncSession, iam: IdentityAccessManager):
    ...
    with pytest.raises(InvalidCurrentPasswordException):
        await iam.login(session=session, email="alice@example.com", password="wrong")


@pytest.mark.asyncio
async def test_login_missing_inputs_raises(session: AsyncSession, iam: IdentityAccessManager):
    with pytest.raises(AssertionError):
        await iam.login(session=session, email=None, password=None)
