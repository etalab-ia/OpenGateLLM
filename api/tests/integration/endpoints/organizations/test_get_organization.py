from unittest.mock import AsyncMock

from httpx import AsyncClient
import pytest
import pytest_asyncio

from api.dependencies import get_one_organization_use_case_factory
from api.domain.organization.errors import OrganizationNotFoundError
from api.infrastructure.fastapi.routes import EndpointRoute
from api.tests.helpers import INVALID_API_KEY, create_key
from api.tests.integration.factories.sql import OrganizationSQLFactory, UserSQLFactory

URL = f"/v1{EndpointRoute.ORGANIZATIONS_ME}"


@pytest.mark.asyncio(loop_scope="session")
class TestGetMyOrganization:
    @pytest_asyncio.fixture(autouse=True)
    async def setup(self, db_session):
        self.organization = OrganizationSQLFactory(name="my-organization")
        self.user = UserSQLFactory(regular_user=True, organization=self.organization)
        self.key = await create_key(db_session, name="user_key", user=self.user, never_expires=True)

    async def test_happy_path(self, client: AsyncClient, db_session):
        response = await client.get(
            url=URL,
            headers={"Authorization": f"Bearer {self.key.token}"},
        )

        assert response.status_code == 200, response.text
        data = response.json()
        assert data["object"] == "organization"
        assert data["id"] == self.organization.id
        assert data["name"] == "my-organization"
        assert data["users"] == 1
        assert isinstance(data["created"], int)
        assert isinstance(data["updated"], int)

        admin_user = UserSQLFactory(admin_user=True)
        admin_key = await create_key(db_session, name="admin_key", user=admin_user, never_expires=True)

        admin_response = await client.get(
            url=URL,
            headers={"Authorization": f"Bearer {admin_key.token}"},
        )

        assert admin_response.status_code == 200, admin_response.text
        admin_data = admin_response.json()
        assert admin_data["object"] == "organization"
        assert admin_data["id"] == admin_user.organization_id
        assert admin_data["users"] == 1

    @pytest.mark.parametrize(
        "use_case_result,expected_status,expected_detail",
        [
            (
                OrganizationNotFoundError(id=999),
                404,
                "Organization 999 not found.",
            ),
        ],
    )
    async def test_error_maps_to_correct_http_status(self, client: AsyncClient, app, use_case_result, expected_status, expected_detail):
        mock_use_case = AsyncMock()
        mock_use_case.execute.return_value = use_case_result
        app.dependency_overrides[get_one_organization_use_case_factory] = lambda: mock_use_case

        response = await client.get(
            url=URL,
            headers={"Authorization": f"Bearer {self.key.token}"},
        )

        assert response.status_code == expected_status
        assert response.json().get("detail") == expected_detail

    @pytest.mark.parametrize(
        "headers,expected_status,expected_detail",
        [
            ({}, 401, "Not authenticated"),
            ({"Authorization": "Bearer malformed-token"}, 401, "Invalid API key."),
            ({"Authorization": f"Bearer {INVALID_API_KEY}"}, 401, "Invalid API key."),
        ],
    )
    async def test_auth(self, client: AsyncClient, headers, expected_status, expected_detail):
        response = await client.get(url=URL, headers=headers)

        assert response.status_code == expected_status
        assert response.json().get("detail") == expected_detail
