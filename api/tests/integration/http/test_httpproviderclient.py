...
class TestHttpProviderClient:
    ...
    async def test_forward_returns_unsupported_provider_endpoint_error_when_adapter_is_missing(self):
        ...
        request = ProviderRequest(endpoint=ProviderEndpoint.OCR)

        result = await http_provider_client().forward(provider=provider, request=request)

        assert result == UnsupportedProviderEndpointError(endpoint=ProviderEndpoint.OCR, provider_type=ProviderType.VLLM)
    ...
    async def test_forward_stream_returns_unsupported_provider_endpoint_error_when_adapter_is_missing(self):
        provider = provider_factory(type=ProviderType.VLLM)
        request = ProviderRequest(id="req-1", endpoint=ProviderEndpoint.OCR)
