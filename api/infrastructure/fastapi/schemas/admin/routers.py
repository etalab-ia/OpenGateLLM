...
from api.domain.router.entities import DEFAULT_QOS_HEALTH_THRESHOLDS, RouterLoadBalancingStrategy, RouterQosMetric
...
class CreateRouterBody(BaseModel):
    ...
    qos_enable: Annotated[bool, Field(default=False, description="Enable QoS admission. When true, retries when providers are full; when false, skips admission.")]  # fmt: off
    qos_retry: Annotated[int, Field(default=10, ge=0, description="Max number of try_admit retries after the first FULL response when qos_enable is true.")]  # fmt: off
    ...
    qos_health_thresholds: Annotated[list[Annotated[float, Field(ge=0, le=2)]], Field(default_factory=lambda: list(DEFAULT_QOS_HEALTH_THRESHOLDS), min_length=2, max_length=2, description="Saturation thresholds [orange, red] as load/qos_limit ratios.")]  # fmt: off
...
class UpdateRouterBody(BaseModel):
    ...
    qos_enable: Annotated[bool, Field(..., description="Enable QoS admission. When true, retries when providers are full; when false, skips admission.")]  # fmt: off
    qos_retry: Annotated[int, Field(..., ge=0, description="Max number of try_admit retries after the first FULL response when qos_enable is true.")]  # fmt: off
    qos_metric: Annotated[RouterQosMetric, Field(..., description="Metric used for QoS load accounting.")]  # fmt: off
    qos_health_thresholds: Annotated[list[Annotated[float, Field(ge=0, le=2)]], Field(..., min_length=2, max_length=2, description="Saturation thresholds [orange, red] as load/qos_limit ratios.")]  # fmt: off
...
class RouterResponse(BaseModel):
    ...
    load_balancing_strategy: Annotated[RouterLoadBalancingStrategy, Field(description="Routing strategy for load balancing between providers of the model. It will be used to identify the model type.", examples=["least_busy"])]  # fmt: off
    qos_enable: Annotated[bool, Field(description="Whether QoS admission is enabled.")]
    qos_retry: Annotated[int, Field(description="Max number of try_admit retries after the first FULL response when qos_enable is true.")]
