...
class CreateRouterBody(BaseModel):
    ...
    router_type: Annotated[RouterType, Field(alias="type", description="Type of the model router. It will be used to identify the model router type.", examples=["text-generation"])]  # fmt: off
...
class UpdateRouterBody(BaseModel):
    name: Annotated[str, StringConstraints(strip_whitespace=True, min_length=1), Field(..., description="Name of the model router.", examples=["model-router-1"])]  # fmt: off
    router_type: Annotated[RouterType, Field(..., description="Type of the model router. It will be used to identify the model router type.", examples=["text-generation"], alias="type")]  # fmt: off
