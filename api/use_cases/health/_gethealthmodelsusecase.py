...
class GetHealthModelsUseCase:
    ...
    def _provider_status(cls, liveness_ok: bool, load: int, qos_limit: int | None) -> HealthStatus:
        ...
        if qos_limit is not None and load >= cls.YELLOW_LOAD_RATIO * qos_limit:
    ...
    async def execute(self, command: GetHealthModelsCommand) -> GetHealthModelsUseCaseResult:
        ...
        for router in routers:
            ...
            for provider in (provider for provider in providers if provider.router_id == router.id):
                ...
                provider_statuses.append(self._provider_status(liveness_ok=liveness_ok, load=loads[provider.id], qos_limit=provider.qos_limit))

            if provider_statuses:
                models.append(ModelHealthStatus(id=router.name, status=max(provider_statuses, key=self.STATUS_PRIORITY.__getitem__)))
