...
from api.domain.provider import ProviderAdapter, ProviderAdapterBuilder, ProviderCapabilities, ProviderClient
...
async def get_provider_capabilities(
    ...
    adapter = provider_adapter_builder.build(endpoint=EndpointRoute.MODELS, provider=provider)

    result = await _get_max_context_length(provider_client=provider_client, adapter=adapter)
    match result:
    ...
    if router_type == RouterType.TEXT_EMBEDDINGS_INFERENCE:
        adapter = provider_adapter_builder.build(endpoint=EndpointRoute.EMBEDDINGS, provider=provider)
        result = await _get_vector_size(provider_client=provider_client, adapter=adapter)
        match result:
            case ProviderNotReachableError() as error:
                return error
            case _:
