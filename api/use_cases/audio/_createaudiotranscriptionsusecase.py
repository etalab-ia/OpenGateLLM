...
class CreateAudioTranscriptionsCommand(CreateAudioTranscriptionsBody):
...
class CreateAudioTranscriptionsUseCase:
    def __init__(
        self,
        ...
        audio_file_size_limit: int | None,
    ) -> None:
    ...
    async def execute(self, command: CreateAudioTranscriptionsCommand) -> CreateAudioTranscriptionsUseCaseResult:
        ...
        if router.type != RouterType.AUTOMATIC_SPEECH_RECOGNITION:
        ...
        adapter = self.provider_adapter_builder.build(endpoint=EndpointRoute.AUDIO_TRANSCRIPTIONS, provider=provider)
        original_request = ProviderOriginalRequest(
            endpoint=EndpointRoute.AUDIO,
            body=CreateAudioTranscriptionsBody(
                input=command.input,
                model=command.model,
