...
from api.use_cases._forwarding import RequestContextCarrier
...
class CreateOCRUseCase:
    ...
    async def execute(self, command: CreateOCRCommand) -> CreateOCRUseCaseResult:
        ...
        original_request = ProviderOriginalRequest(
            ...
            body=CreateOCRBody(**command.model_dump(exclude={"request_context"})),
        )
        ...
        match result:
            case ProviderFormattedResponse() as formatted_response:
                completion_tokens = self.model_tokenizer.compute_tokens(texts=self._extract_output_texts(formatted_response.data))
