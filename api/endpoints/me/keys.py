...
@router.delete(path=EndpointRoute.KEYS + "/{key}", dependencies=[Security(dependency=AccessController())], status_code=204)
...
@router.get(path=EndpointRoute.KEYS + "/{key}", dependencies=[Security(dependency=AccessController())], status_code=200, response_model=Key)
async def get_key(
    request: Request,
