...
async def _setup_elasticsearch_vector_store(configuration: Configuration, global_context: GlobalContext):
    ...
    if router is None:
        logger.warning("You defined Elasticsearch as vector store but no embeddings router was found. Document management features will be disabled.")
        global_context.elasticsearch_vector_store = None
        return
    ...
    global_context.document_manager = DocumentManager(vector_store_model=router.name, parser_manager=parser_manager)
...
async def _setup_parser(configuration: Configuration, global_context: GlobalContext):
    if configuration.dependencies.parser is None:
        ...
        check_health = parser.check_health()
