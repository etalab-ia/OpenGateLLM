...
def apply_sync_qos_policy(provider_id: int, qos_metric: MetricType, qos_value: float | None, redis_client: Redis) -> bool:
    can_be_forwarded = True
    if qos_metric == MetricType.INFLIGHT:
        inflight_requests = redis_client.get(f"{METRIC__GAUGE_PREFIX}:{MetricType.INFLIGHT.value}:{provider_id}")
        if inflight_requests is not None and qos_value is not None:
...
async def apply_async_qos_policy(provider_id: int, qos_metric: MetricType, qos_value: float | None, redis_client: AsyncRedis) -> bool:
    can_be_forwarded = True
    if qos_metric == MetricType.INFLIGHT:
