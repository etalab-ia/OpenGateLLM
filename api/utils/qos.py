...
def apply_sync_qos_policy(
    provider_id: int, qos_metric: MetricType | None, qos_threshold: float | None, performance_indicator: float | None, redis_client: Redis
) -> bool:
    ...
    if qos_metric is not None and qos_metric == MetricType.INFLIGHT:
        ...
        if inflight_requests is not None and qos_threshold is not None:
            inflight_requests = int(inflight_requests)
            if inflight_requests > qos_threshold:
...
async def apply_async_qos_policy(
    provider_id: int, qos_metric: MetricType | None, qos_threshold: float | None, performance_indicator: float | None, redis_client: AsyncRedis
) -> bool:
    can_be_forwarded = True
    if qos_metric is not None and qos_metric == MetricType.INFLIGHT:
